...
def generate_synthetic_data(num_patients: int, num_stations: int, seed: int, output_dir: str | Path) -> None:
    ...
    patients = pd.DataFrame({
        'patient_id': [f'SP{i:06d}' for i in range(1, num_patients + 1)],
    ...
    pathways.to_csv(output_dir / 'synthetic_pathways.csv', index=False)
    resources.to_csv(output_dir / 'synthetic_resources.csv', index=False)
    services.to_csv(output_dir / 'synthetic_services.csv', index=False)
